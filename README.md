# Itransition---Data-Engineering

## TASK #1_DATA (for DATA group)
...
- average_price of books published that year in USD rounded to cents using conversion rate €1 = $1.2

You can use any language for uploading data, but transformation is supposed to be inside the database.

Record the video demonstrating all the code you've wrote (including SQL query that builds the summary table) as well as data in the RDBMS with the row count (both tables). Don't forget to record the content of the summary table.
